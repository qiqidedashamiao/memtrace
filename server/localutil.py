...
def trim_binary_data(binary_data):
    ...
    first_crlf_pos = binary_data.find(b'\r\n')
    # 查找第二个 \r\n 的位置
    second_crlf_pos = binary_data.find(b'\r\n', first_crlf_pos + 2)
    # 查找第三个 \r\n 的位置
    third_crlf_pos = binary_data.find(b'\r\n', second_crlf_pos + 2)
    # 查找最后一个 \r\n 的位置
    last_crlf_pos = binary_data.rfind(b'\r\n')
    # 查找倒数第二个 \r\n 的位置
    second_last_crlf_pos = binary_data.rfind(b'\r\n', 0, last_crlf_pos)
    # 如果找到了 \r\n, 则进行截取
    if first_crlf_pos != -1 and second_crlf_pos != -1 and third_crlf_pos != -1 and second_last_crlf_pos != -1 and last_crlf_pos != -1:
        # 截取第三个 \r\n 后和倒数第二个 \r\n 前的数据
        trimmed_data = binary_data[third_crlf_pos + 2:second_last_crlf_pos]
        return trimmed_data
    else:
        # 如果没有找到，或数据无效，返回原数据
        return binary_data
